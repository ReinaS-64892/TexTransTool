...
namespace net.rs64.TexTransTool.TextureAtlas
{
    ...
    public sealed partial class AtlasTexture : TexTransRuntimeBehavior , ITexTransToolStableComponent
    {
        ...
        internal override void Apply(IDomain domain)
        {
            ...
            var nowRenderers = GetAtlasAllowedRenderers(domain.EnumerateRenderer());
            ...
            var targetRenderers = FilterTargetRenderers(targeting, nowRenderers, targetMaterials, atlasSetting.IncludeDisabledRenderer);
            ...
            //Texture Fine Tuning
            var tunedAtlasTextures = DoTextureFinTuning(engine, atlasContext, atlasSetting, compiledAtlasTextures, experimentalOptions);
        ...
        internal static Renderer[] FilterTargetRenderers(IRendererTargeting targeting, List<Renderer> nowRenderers, HashSet<Material> targetMaterials, bool includeDisabledRenderer)
        {
            return nowRenderers
                ...
                ).Where(r => CheckRendererActive(targeting, r, includeDisabledRenderer))
                .ToArray();
        }
        ...
        internal static TexFineTuningResult DoTextureFinTuning(ITexTransToolForUnity engine, AtlasContext atlasContext, AtlasSetting atlasSetting, Dictionary<string, ITTRenderTexture> compiledAtlasTextures, AtlasTextureExperimentalFeature? experimentalOptions)
        {
            ...
            // AutoTunings
            if (experimentalOptions?.AutoTextureSizeSetting ?? false) SetSizeDataMaxSize(atlasTexFineTuningTargets, atlasContext.MaterialGroupingCtx);
            if (experimentalOptions?.AutoMergeTextureSetting ?? false) DefaultMargeTextureDictTuning(atlasTexFineTuningTargets, atlasContext.MaterialGroupingCtx);
            if (experimentalOptions?.AutoReferenceCopySetting ?? false) DefaultRefCopyTuning(atlasTexFineTuningTargets, atlasContext.MaterialGroupingCtx);

            foreach (var fineTuning in atlasSetting.TextureFineTuning) { fineTuning?.AddSetting(atlasTexFineTuningTargets); }
            if (experimentalOptions is not null) IndividualTuningAddSettings(atlasTexFineTuningTargets, experimentalOptions.TextureIndividualFineTuning);
        ...
        internal static bool CheckRendererActive(IRendererTargeting targeting, Renderer r, bool includeDisabledRenderer)
        {
            ...
            {
                if (targeting.IsEnable(r) is false) { return false; }
                if (targeting.IsActive(r.gameObject) is false) { return false; }
            }
            return true;
        }
        internal static List<Renderer> GetAtlasAllowedRenderers(IEnumerable<Renderer> domainRenderers)
        { return domainRenderers.Where(i => IsAtlasAllowedRenderer(i)).ToList(); }
        internal static bool IsAtlasAllowedRenderer(Renderer item)
        {
        ...
        internal override IEnumerable<Renderer> ModificationTargetRenderers(IRendererTargeting rendererTargeting)
        {
            ...
            var nowRenderers = GetAtlasAllowedRenderers(rendererTargeting.EnumerateRenderer());
            ...
            return FilterTargetRenderers(rendererTargeting, nowRenderers, targetMaterials, isIncludeDisable);
