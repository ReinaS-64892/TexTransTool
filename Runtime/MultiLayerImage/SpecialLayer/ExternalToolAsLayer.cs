...
namespace net.rs64.TexTransTool.MultiLayerImage
{
    ...
    */
    ...
    public sealed class ExternalToolAsLayer : AbstractLayer
    {
        ...
        static Type ColorChangerForUnity => Type.GetType("net.puk06.ColorChanger.ColorChangerForUnity, net.puk06.color-changer.runtime");
        static Type TextureUtils => Type.GetType("net.puk06.ColorChanger.Utils.TextureUtils, net.puk06.color-changer.editor");

        // ColorChangerForUnity としての動作を止めます。
        void Start()
        {
            if (ColorChangerForUnity is null) { return; }
            var cc4u = GetComponent(ColorChangerForUnity) as MonoBehaviour;
            if (cc4u == null) { return; }
            cc4u.enabled = false;
        }
        new void OnDestroy()
        ...
        internal override LayerObject<ITexTransToolForUnity> GetLayerObject(GenerateLayerObjectContext ctx)
        {
            ...
            if (ColorChangerForUnity is null || TextureUtils is null)
            {
                TTLog.Info("ColorChangerForUnity is not compatible version!");
                return new EmptyLayer<ITexTransToolForUnity>(Visible, lm, alphaOperator, Clipping, blKey);
            }
            if (engine is not TTCEUnity)
            {
                TTLog.Info("ColorChangerForUnityAsLayer is run only on UnityBackend");
                return new EmptyLayer<ITexTransToolForUnity>(Visible, lm, alphaOperator, Clipping, blKey);
            }

            domain.ObserveToChildeComponents<MonoBehaviour>(gameObject);//治安悪いが ... Generic 版しか無いから仕方がない
            var cc4u = GetComponent(ColorChangerForUnity);
            if (cc4u == null)
            {
                TTLog.Info("ColorChangerForUnity is not compatible version!");
                return new EmptyLayer<ITexTransToolForUnity>(Visible, lm, alphaOperator, Clipping, blKey);
            ...
            var grabBlend = new ColorChangerForUnityAsGrabBlending(cc4u);
            return new GrabBlendingAsLayer<ITexTransToolForUnity>(Visible, lm, Clipping, blKey, grabBlend);
        }
        public class ColorChangerForUnityAsGrabBlending : ITTGrabBlending
        {
            private readonly Component cc4U;

            public ColorChangerForUnityAsGrabBlending(Component cc4u)
            {
                cc4U = cc4u;
            }
            public void GrabBlending<TTCE>(TTCE engine, ITTRenderTexture grabTexture)
            ...
            , ITexTransDriveStorageBufferHolder
            {
                ...
                using var sourceRt = ttceUnity.CreateRenderTexture(grabTexture.Width, grabTexture.Hight, grabTexture.ContainsChannel);
                ttceUnity.CopyRenderTexture(sourceRt, grabTexture);

                var funcInfo = TextureUtils.GetMethod("ProcessTexture", BindingFlags.Static | BindingFlags.NonPublic);
                if (funcInfo is null) // unknown CC4U
                {
                    TTLog.Info("ColorChangerForUnity is not compatible version!");
                    return;
                }
                var genericMethod = funcInfo.MakeGenericMethod(typeof(RenderTexture));
                if (genericMethod is null) // unknown CC4U
                {
                    TTLog.Info("ColorChangerForUnity is not compatible version!");
                    return;
                }

                genericMethod.Invoke(null, new object[3] { ((UnityRenderTexture)sourceRt).RenderTexture, ((UnityRenderTexture)grabTexture).RenderTexture, cc4U });
