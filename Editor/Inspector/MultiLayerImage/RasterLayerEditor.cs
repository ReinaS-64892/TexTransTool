...
namespace net.rs64.TexTransTool.Editor.MultiLayerImage
{
    ...
    internal class RasterImportedLayerEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (targets.Length != 1) { return; }
            if (thisTarget.ImportedPNG != null && thisTarget.ImportedPNG.PreviewTexture != null)
            {
                EditorGUI.DrawTextureTransparent(EditorGUILayout.GetControlRect(GUILayout.Height(400)), thisTarget.ImportedPNG.PreviewTexture, ScaleMode.ScaleToFit);
            }
        }
